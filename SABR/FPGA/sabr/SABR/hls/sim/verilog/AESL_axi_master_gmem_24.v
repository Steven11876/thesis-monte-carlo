...
//------------------------Parameter----------------------
`define TV_IN_gmem_24 "../tv/cdatafile/c.SABR.autotvin_gmem_24.dat"
`define TV_OUT_gmem_24 "../tv/rtldatafile/rtl.SABR.autotvout_gmem_24.dat"
 parameter gmem_24_ADDR_BITWIDTH = 32'd 64;
 parameter gmem_24_AWUSER_BITWIDTH = 32'd 1;
 parameter gmem_24_DATA_BITWIDTH = 32'd 128;
...
parameter gmem_24_C_DATA_BITWIDTH = 32'd 128;
parameter gmem_24_transaction_depth = 32'd 51;
parameter gmem_24_mem_depth = 32'd 51;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_24_mem_depth) begin
                $display ("C:/Users/steve/thesis-monte-carlo/SABR/FPGA/sabr/SABR/hls/sim/verilog/AESL_axi_master_gmem_24.v: Write request address %d exceed AXI master gmem_24 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_24_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_24_mem_depth) begin
                $display ("C:/Users/steve/thesis-monte-carlo/SABR/FPGA/sabr/SABR/hls/sim/verilog/AESL_axi_master_gmem_24.v: Read request address %d exceed AXI master gmem_24 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_24_mem_depth); 
                $finish;
            end
...
integer gmem_24_port2_fresh_depth[0:1];
integer gmem_24_port2_blank_depth[0:1];

initial begin : runtime_depth_init_process
    gmem_24_port0_fresh_depth[0] = 1;
    gmem_24_port0_blank_depth[0] = 0;
    gmem_24_port1_fresh_depth[0] = 1;
    gmem_24_port1_blank_depth[0] = 0;
    gmem_24_port2_fresh_depth[0] = 49;
    gmem_24_port2_blank_depth[0] = 0;
end
//------------------------Read file------------------------ 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem_24_transaction_depth; i = i + 1) begin 
          ...
          ret = $fread(token_tmp,fp);
              ...
              if (port_idx == 2) begin
                  if (port_read_done[port_idx] == 1'b0 && read_idx == gmem_24_port2_fresh_depth[transaction_num]) begin
                      port_read_done[port_idx] = 1'b1;
                      read_idx = 0;
                  end
                  if (port_read_done[port_idx] == 1'b0 && read_idx < gmem_24_port2_fresh_depth[transaction_num]) begin
          ret = $fread(token_tmp,fp);
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_read_done[port_idx] == 1'b1 && read_idx == gmem_24_port2_blank_depth[transaction_num]) begin
                      port_fill_done[port_idx] = 1'b1;
                      port_idx = port_idx + 1;
                      read_idx = 0;
                  end
  ...
  while(1) begin 
      ...
      for(i = 0; i < gmem_24_mem_depth; i = i + 1) begin 
          ...
          for (j=0;j<1;j=j+1) begin
              ...
              if (port_idx == 2) begin
                  if (port_write_done[port_idx] == 1'b0 && write_idx == gmem_24_port2_fresh_depth[transaction_num]) begin
                      port_write_done[port_idx] = 1'b1;
                      write_idx = 0;
                      write_ready = 1'b0;
                  end
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_write_done[port_idx] == 1'b1 && write_idx == gmem_24_port2_blank_depth[transaction_num]) begin
                      write_idx = 0;
                      port_fill_done[port_idx] = 1'b1;
                      port_idx = port_idx + 1;
                  end
